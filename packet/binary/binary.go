...
func ReadFrom(conn *net.UDPConn, order ByteOrder, data any) (net.Addr, error) {
	// Fast path for basic types and slices.
	if n := intDataSize(data); n != 0 {
		bs := make([]byte, n)
		if _, addr, err := conn.ReadFrom(bs); err != nil {
			return addr, err
		}
		switch data := data.(type) {
		case *bool:
	...
	if size < 0 {
		return nil, errors.New("binary.Read: invalid type " + reflect.TypeOf(data).String())
	}
	d := &decoder{order: order, buf: make([]byte, size)}
	_, addr, err := conn.ReadFrom(d.buf)
	if err != nil {
		return addr, err
	}
